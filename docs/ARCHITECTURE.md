...
| `trigrid.py` | **Which triangles are near this query?** — a uniform grid over a triangle soup, shared by the two things that interrogate a level's collision mesh many times a frame: the character controller (`body.py`, by box) and `raycast.py` (by ray). Answering either by looking at every triangle is O(T) per query, which at a real map's 66k triangles is milliseconds a cast — enough that a few bots do not fit in a frame. Asking **along a ray** rather than by the ray's bounding box is the point of the second query: a cast from one end of a level to the other has a box containing the whole level, so only walking the cells the ray actually enters makes a long cast cheap. A query is never allowed to lose a triangle it touches — the grid narrows and the caller still does the exact test — so its contract is a *superset*, and the only other thing that matters is that the superset is small. Triangles spanning very many cells are not binned at all but kept on a list every query looks at, because one triangle can span a level. The two queries differ in how they finish: a **ray** filters its candidates by the query's own bounds, which is semantics rather than speed — it is what stops a cast answering with triangles past its own limit — while a **box** filters only once it has been handed enough to be worth it (`FILTER_ABOVE`). A box's contract is a superset the caller must test exactly anyway, and the filter costs two gathers and four reductions *however few* candidates there are — so for a box in open space, touching one or two cells, it is pure overhead against a test costing a fraction of a microsecond a triangle. In a real level's geometry the same box is handed hundreds of triangles and keeps a handful, and there those six array operations are cheap against the hundreds of exact tests they save; measured through twig-bb on `oa_dm3` with ten characters walking, filtering above the threshold is 21.6ms a frame against 18.7. They also differ in whether the cells are **merged into a unique set**: a triangle large enough to lie in several of the cells a query touches is named once per cell, and for anything that *counts* what it found naming it twice is a wrong answer rather than a slow one. A ray does not count — it keeps the nearest of whatever it meets, and meeting one triangle twice yields the same distance and the same triangle — so it pays for the extra exact test rather than for the sort, which on a level's geometry is the cheaper of the two. |
...
| `threaded.py` | `ThreadedSimulation` — runs `step()` on a daemon thread, publishes snapshots. The schedule it keeps is `pace()`, a plain function of the deadline, the clock and the tick interval: a tick with time to spare waits out the remainder and advances the deadline by exactly one interval, so jitter does not accumulate into drift, while one that overruns waits not at all, counts a drop and moves the deadline to the present — a machine already too slow cannot pay back the ticks it missed, and running them back to back is the spiral. It is a function rather than a stretch of the loop so that the rule can be checked against clock readings chosen for it, instead of against whether an operating system happened to hand a thread its turn on time. |
